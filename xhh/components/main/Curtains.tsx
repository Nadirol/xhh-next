    ...
    return (
            <div className="flex gap-10 flex-col py-6 md:py-12 w-container-large mx-auto">
                ...
                <div className="pb-8 border-b border-neutral-200 flex items-start md:items-center justify-between -md:flex-col">
                    <h2 className="text-red-500 font-bold text-2xl md:text-[4rem] w-2/3 md:tracking-[0.2rem]">{t("curtains")}</h2>
                    <div className="flex gap-4 flex-col md:items-end">
                        <Link href={`/${i18n?.language}/products?category=curtain`} className="text-neutral-800 md:text-xl underline hover:text-red-500">
                            {t('viewMore')}
                        </Link>
                        <a href={i18n?.language === 'en' ? "/assets/XHH-Catalogue-en.pdf" : "/assets/XHH-Catalogue-vi.pdf" } target="_blank"
                        className="text-neutral-50 bg-red-500 px-8 py-3 font-semibold whitespace-nowrap	">
                            {t('downloadCatalogue')}
                        </a>
                    </div>
                </div>
